...
task PackageWarrantyUI -depends SetReleaseBuild, Clean, CommonAssemblyInfo, Compile {
	...
	#pstrami deployment
	copy_files "$base_dir\deployment\warranty" "$temp_package_dir"
...
function deploy-database($action,$server,$db_name,$scripts_dir) {
	$roundhouse_version_file = "$source_dir\Warranty.Server\bin\$project_config\Warranty.Server.dll"
...
function start-cassini {
	&$cassini_exe "/port:$admin_port" "/path:$warranty_web_dir"
}

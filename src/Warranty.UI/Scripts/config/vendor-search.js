define(['urls', 'text!templates/vendor-search-item.html', 'bloodhound'], function (urls, template, bloodhound) {
    return {
        ...
        engine: new Bloodhound({
            ...
            remote: urls.QuickSearch.Vendors
            + '?query=%QUERY'
            + '&cityCode=' + (($('#vendor-search').attr('data-city-code') === undefined) ? '' : $('#vendor-search').attr('data-city-code'))
            + '&invoicePayableCode=' + (($('#vendor-search').attr('data-ip-code') === undefined) ? '' : $('#vendor-search').attr('data-ip-code'))
        })
    };
});

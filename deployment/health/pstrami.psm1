$remoteExecutionSuccessMessage = "Deployment Succeeded"
$assertPrefix = "ERROR: "
...
function Install-RemoteServer{
    ...
    write-host "`nInstall-RemoteServer $servername $environment"
    ...
    $success = (select-string -casesensitive -inputobject $result -pattern $remoteExecutionSuccessMessage)
    $failure = (select-string -inputobject $result -pattern $assertPrefix)
...
function Invoke-RemoteCommand{
    ...
    write-host "Invoke-RemoteCommand $server : $cmd"
...
function Install-LocalServer {
    ...
    write-host $remoteExecutionSuccessMessage
}
...
function Load-Configuration {
    param(
        [string] $configFile=".\pstrami.config.ps1"
    )
...
function Role {
    param(
        ...
        [Parameter(Position=1,Mandatory=1)] [scriptblock] $incremental = $null, 
        [Parameter(Position=1,Mandatory=1)] [scriptblock] $fullinstall = $null
    )
...
function Server {
    param(
        ...
        [Parameter(Position=1,Mandatory=1)] [string[]] $roles = $null,
        [string] $credential=""
    )
...
function Environment {
    param(
        [Parameter(Position=0,Mandatory=1)] [string] $name, 
        [Parameter(Position=1,Mandatory=1)] [object[]] $servers,
        [string] $installPath
    )
...
function Assert {
    ...
    if (!$conditionToCheck) { throw "$assertPrefix $failureMessage" }
}
